...
@section('textocarta')
    ...
    <br><br>
    <div class="c"><strong>“Por la cual se concede nombramiento Provisional por licencia de maternidad <br> de la Delegación del {{$usuario->departamentonombre}}”</strong></div>
    <br><br>
    ...
    <br>
    <div style="text-align: justify;"> Que el literal c) del artículo 20 de la Ley 1350 de 2009, establece: <br><br> (...) <br><br> c) Nombramiento provisional discrecional: Esta clase de nombramiento es excepcional y sólo procederá por especiales razones del servicio. El término de la provisionalidad se podrá hacer hasta por seis (6) meses improrrogables, deberá constar expresamente en la providencia de nombramiento. En el transcurso del término citado se deberá abrir el concurso respectivo para proveer el empleo definitivamente; <br><br>(…) <br><br>Que igualmente el Articulo 65 de la ley 1350 de 2009, establece: <br><br> “A partir de la vigencia de la presente ley, la Registraduria Nacional realizará las acciones necesarias para poner en práctica el sistema de carrera especial, que deberá operar plenamente dentro de los 24 meses siguientes a la entrada en vigencia de la presente ley <br><br> Que, el artículo 276 de la Ley 1450 de 2011, amplió hasta el 6 de agosto de 2012, el término para la implementación del sistema de carrera especial de la Entidad, establecido en el artículo 65 de la Ley 1350 de 2009. <br><br>Que, la Registraduría Nacional del Estado Civil, se encuentra adelantando las actividades de la primera fase del proyecto para la implementación de los procesos de selección en el marco del sistema especial de carrera. <br><br> Que, existe una vacante por licencia de maternidad en el cargo de {{$dato->cargofunc}} {{$dato->codigofunc}}-{{$dato->gradofunc}} en la planta de la Delegacion Departamental del {{$usuario->departamentonombre}}. <br><br> Que, en virtud de lo anterior los señores Delegados Departamentales, solicitaron la viabilidad para nombrar provisionalmente al señor(a) <strong>{{strtoupper($dato->nombrefunc1)}}</strong> identificada con Cedula de Ciudadania No. {{number_format($dato->cedulafuncionario, 0, ',', '.')}},  con el fin de tomar posesión en el cargo de <strong> {{$dato->cargofunc}} {{$dato->codigofunc}}-{{$dato->gradofunc}} </strong> en la {{$dato->nombreoficina}} - {{$dato->nombredepartamento}}<br><br> Que mediante aprobacion de Viabilidad de fecha {{ \Carbon\Carbon::parse($dato->fechaviavilidad)->locale('es')->isoFormat('LL') }} , enviada por el despacho del Gerente del Talento Humano, se autorizo nombrar al Funcionario(a) <strong>{{strtoupper($dato->nombrefunc1)}}</strong> identificada con Cedula de Ciudadania No. {{number_format($dato->cedulafuncionario, 0, ',', '.')}} a tomar posesión en el cargo de <strong> {{$dato->cargofunc}} {{$dato->codigofunc}}-{{$dato->gradofunc}} </strong> en la {{$dato->nombreoficina}} - {{$dato->nombredepartamento}} por el tiempo que dure la licencia de Maternidad del servidora <strong> {{strtoupper($dato->nombrefunc2)}}</strong> <br><br>Que, por lo anteriormente expuesto,</div>
    <div class="c"><strong>RESUELVEN:</strong></div><br>
    <div style="text-align: justify;"><strong>ARTÍCULO PRIMERO: Nombrar,</strong> A partir del {{ \Carbon\Carbon::parse($dato->fechainicial)->locale('es')->isoFormat('LL') }}, provisionalmente de manera discrecional en la planta global de la Delegacion del {{ ucwords(strtolower($dato->nombredepartamento)) }} a la Señor(a) <strong>{{strtoupper($dato->nombrefunc1)}}</strong> identificada con Cedula de Ciudadania No. {{number_format($dato->cedulafuncionario, 0, ',', '.')}}, en el cargo de <strong> {{$dato->cargofunc}} {{$dato->codigofunc}}-{{$dato->gradofunc}} </strong> en la {{$dato->nombreoficina}} - {{$dato->nombredepartamento}}, con una asignación básica mensual de ${{number_format($dato->sueldofunc, 0, ',', '.')}}. <br><br><strong> PARAGRAFO:</strong> la duración de este nombramiento provisional será hasta que dure la licencia de Maternidad de la Funcionaria <strong>{{strtoupper($dato->nombrefunc2)}}</strong>, a partir de la fecha de posesión </div>
    <br>
