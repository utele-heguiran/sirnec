...
@foreach ($user as $usuario)@endforeach
    ...
    <div>
        ...
        <br>
        Oficio DDV - 0910 - 26 - 548  - {{ $numoficio }}<br>
         <br>
        <span class="n">Dr.(a) </span><br>
        <span class="t n"> {{ $registro[0]->name }} </span><br>
        <span>Direccion: {{ $registro[0]->direccion }} </span><br>
        <span>Codigo Postal : {{ $registro[0]->codigopostal }} </span><br>
        <span>{{ $registro[0]->departamentonombre }} </span><br>    

        <p><span class="n">Asunto : </span> Material Rechazado de la Oficina :  Registraduria {{ $registro[0]->nameoficina }}</p>
        ...
        <table >
            <tbody>
                ...
                @foreach ($reporte as $registro)
                    <tr>
                        <td class="f">{{ $registro[0]->numpreparacion }}</td>
                        <td class="f">{{ $registro[0]->numdocumento }}</td>
                        <td class="f">{{ $registro[0]->nameclasdevolucion }}</td>
                        <td class="f">{{ $registro[0]->nombretipotramite }}</td>
                        <td class="f">{{ $registro[0]->codpmt }}</td>
                        <td class="f">{{ $registro[0]->fecpreparacion }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
